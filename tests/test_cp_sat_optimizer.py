...
def test_candidate_generation_dataset_v1_has_multiple_windows_and_overnight_support():
    ...
    assert len(jobs)==40 and candidates
...
def test_optimizer_dataset_v1_outputs_valid_plan(tmp_path):
    ...
    assert len(plan)==40 and (output/"optimized_maintenance_plan.csv").exists() and (output/"plan_comparison.csv").exists() and (output/"optimization_summary.json").exists()

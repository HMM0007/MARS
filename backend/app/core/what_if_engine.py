...
def _build_blockers(scenario: Dict[str, Any], tracks: List[str]) -> List[TrainMovement]:
    ...
    return [TrainMovement(train_id=f"WHATIF-BLOCK-{i}-{track}", train_number="WHAT-IF-BLOCK", train_name="Scenario Occupancy Window", train_type="FREIGHT", section_id=scenario["section_id"], track_id=track, entry_time=start, exit_time=end, priority=999, direction="UP", is_fixed=True) for i, track in enumerate(tracks, 1)]
...
def _build_freight_surge(trains: List[TrainMovement], scenario: Dict[str, Any], percent: int) -> List[TrainMovement]:
    ...
    if not overlapping:
        raise HTTPException(status_code=422, detail={"error": "SURGE_WINDOW_EMPTY", "message": "The selected window does not overlap timetable freight movements."})
...
def simulate_scenario(scenario: Dict[str, Any], approved: Dict[str, Any]) -> Dict[str, Any]:
    ...
    end_time = start_time + timedelta(minutes=int(scenario["duration_minutes"]))
    ...
    deferred_ids = {str(v.get("job_id")) for v in result.get("deferred_jobs", []) if isinstance(v, dict) and v.get("job_id")}
    baseline_deferred_ids = {str(v.get("job_id")) for v in baseline_plan.get("deferred_jobs", []) if isinstance(v, dict) and v.get("job_id")}
    ...
    impact = {"jobs_affected": len(impacted_ids), "blocks_affected": abs(len(result.get("scheduled_blocks", [])) - len(baseline_blocks)) + len(moved), "jobs_delayed_or_moved": len(moved), "jobs_deferred": len(newly_deferred), "additional_deferrals": len(newly_deferred), "baseline_scheduled_jobs": len(baseline_ids), "scenario_scheduled_jobs": len(scenario_ids), "solver_status": result.get("status", result.get("solver_status", "UNKNOWN"))}
    impact["severity"] = _severity(impact)

    scenario_label = {"TRACK_OUTAGE": "Track outage", "SECTION_OUTAGE": "Section outage", "EMERGENCY_BLOCK": "Emergency block", "FREIGHT_SURGE": f"Freight demand surge (+{scenario['impact_percent']}%)", "MONSOON_SLOWDOWN": f"Monsoon weather slowdown ({scenario['impact_percent']}%)"}[scenario_type]
    details = [f"{scenario_label} assessed for {requested_section}.", f"Scenario window: {start_time.isoformat(timespec='minutes')} to {end_time.isoformat(timespec='minutes')}."]
    if scenario_type == "FREIGHT_SURGE":
        details.append("Additional freight paths are modelled in memory using the existing timetable demand shape; source data is unchanged.")
    elif scenario_type == "MONSOON_SLOWDOWN":
        details.append("Weather stress is modelled as conservative occupancy-time extension; it is a scenario proxy, not a live speed restriction feed.")
    if moved:
    ...
    return {"status": "SIMULATED", "baseline_revision": approved.get("revision"), "planning_week": week, "scenario": {**scenario, "start_time": start_time.isoformat(), "end_time": end_time.isoformat(), "blocked_tracks": blocker_tracks}, "impact": impact, "affected_job_ids": impacted_ids, "deferred_job_ids": sorted(deferred_ids), "operational_impact": details, "scenario_solver": {"status": result.get("status", result.get("solver_status", "UNKNOWN")), "scheduled_blocks": len(result.get("scheduled_blocks", [])), "model": "HardenedWeeklyCPSATSolver", "non_destructive": True}}

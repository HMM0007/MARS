...
export default function CorridorMapPage() {
  ...
  return <main className={`flex min-h-full flex-col gap-3 bg-[#F4F6F8] p-4 sm:p-5 ${fullScreen ? 'relative z-[9998] min-h-screen' : ''}`}>
    {!fullScreen && <>
    ...
    {fullScreen && <div className="fixed left-4 top-4 z-[10001] rounded-md border border-[#D6DEE6] bg-white/95 px-3 py-2 shadow-lg"><div className="text-[9px] font-bold uppercase tracking-[0.16em] text-[#8796A5]">Central Railway • Pune Division</div><div className="text-sm font-bold text-[#173E6C]">Pune–Lonavala Corridor</div></div>}
    ...
    <section className={fullScreen ? 'fixed inset-0 z-[10000] bg-[#F4F6F8]' : 'min-h-0 flex-1'}><SatelliteMap blocks={blocks} jobs={jobs} selectedBlock={selectedBlock} selectedJob={selectedJob} onSelectBlock={setSelectedBlock} onSelectJob={setSelectedJob} isFullScreenMode={fullScreen} onToggleFullScreen={() => setFullScreen((v) => !v)} /></section>

...
const fields = [
  ['job_id', 'Job ID', 'ENG-NEW-001'], ['asset_id', 'Asset ID', 'TRK-PUNE-001'],
  ['asset_type', 'Asset Type', 'TRACK'], ['section_id', 'Section', 'PUNE-LNL'],
  ['track_id', 'Track', 'PUNE-LNL-UP'], ['location_km', 'Location (KM)', '227.4'],
  ['defect_type', 'Maintenance / Defect', 'TRACK_RENEWAL'], ['estimated_duration_hours', 'Duration (hours)', '2'],
];
...
export default function EngineeringDashboardPage() {
  ...
  return <main className="p-4 space-y-4 bg-[#F4F6F8] min-h-full font-sans text-[#1F2933]">
    ...
    {showAdd && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><form onSubmit={submitJob} className="bg-white rounded-lg shadow-2xl border border-[#D6DEE6] w-full max-w-2xl overflow-hidden">
      <div className="px-5 py-4 text-white" style={{backgroundColor: '#1E3A5F'}}><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-black uppercase tracking-wide">Add Engineering Maintenance Job</h2><p className="text-[10px] text-white/75 mt-1">Submit to the Unified Job Pool • Planner schedules the block</p></div><button type="button" onClick={()=>setShowAdd(false)} className="text-white/80 hover:text-white" aria-label="Close"><span className="text-lg leading-none">×</span></button></div></div>
      <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{fields.map(([name,label,placeholder])=><label key={name} className="text-[10px] font-bold uppercase tracking-wide text-[#52606D]">{label}<input name={name} placeholder={placeholder} required type={name==='location_km'||name==='estimated_duration_hours'?'number':'text'} step={name==='location_km'||name==='estimated_duration_hours'?'0.1':undefined} className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933] outline-none focus:border-[#3B6EA5]"/></label>)}</div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D]">Criticality<select name="criticality_level" defaultValue="HIGH" className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933]"><option>CRITICAL</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select></label>
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D]">Due Date<input name="due_date" type="date" defaultValue="2026-09-15" required className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933]"/></label>
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D]">Preferred Window<select name="preferred_window" defaultValue="NIGHT" className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933]"><option>NIGHT</option><option>DAY</option><option>ANY</option></select></label>
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D]">Machine Required<select name="machine_required" defaultValue="NO" className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933]"><option>NO</option><option>YES</option></select></label>
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D] md:col-span-2">Dependency Job ID<input name="dependency_job_id" placeholder="e.g. SNT-PLN-044 or leave blank" className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal text-[#1F2933]"/></label>
          <label className="text-[10px] font-bold uppercase tracking-wide text-[#52606D] md:col-span-2">Safety Conflict Tag<select name="safety_conflict_tag" defaultValue="NORMAL" className="mt-1 w-full border border-[#D6DEE6] rounded px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-[#1F2933]"><option>NORMAL</option><option>INTERLOCKING</option><option>LEVEL_CROSSING</option><option>OHE_ISOLATION</option></select></label>
        </div>
      </div>
      <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-[#D6DEE6] bg-[#F8FAFB]"><button type="button" onClick={()=>setShowAdd(false)} className="px-3 py-2 border border-[#D6DEE6] bg-white rounded text-xs font-semibold text-[#52606D]">Cancel</button><button type="submit" className="px-4 py-2 rounded text-xs font-bold text-white" style={{backgroundColor: '#3B6EA5'}}>Submit Job to Intake</button></div>
    </form></div>}
  </main>;
}

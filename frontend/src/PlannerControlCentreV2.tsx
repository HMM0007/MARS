...
import { checkAllocation, fetchConflicts, fetchJobs, fetchNotifications, fetchPlanBlocks, fetchPlanJobs, fetchPlanSummary, fetchPlans, fetchTrains, generateOptimizationPlan, triggerReplanning } from './services/api'
...
export default function PlannerControlCentreV2({ session, onLogout }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [jr, pj, br, tr, cr, nr, sr, pr] = await Promise.all([
        fetchJobs({ department: dept }), fetchPlanJobs({ department: dept || '' }), fetchPlanBlocks(), fetchTrains(), fetchConflicts(), fetchNotifications('Divisional Planner'), fetchPlanSummary(), fetchPlans(),
      ])
      setJobs(jr?.jobs || []); setPlanJobs(pj?.jobs || []); setBlocks(br?.blocks || []); setTrains(tr?.trains || []); setConflicts(cr?.conflicts || []); setNotifications(nr?.notifications || []); setSummary(sr || {}); setReview(pr?.review || { status: 'PENDING_REVIEW' })

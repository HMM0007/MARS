...
export default function PlannerControlCentreV2({ session, onLogout }: Props) {
  ...
  return <div className="pc2-shell">
    <aside className="pc2-sidebar"><div className="pc2-brand"><div className="pc2-rail">रेल</div><div><strong>MARS</strong><small>Maintenance Allocation &amp; Routing System</small></div></div><div className="pc2-role">DIVISIONAL PLANNER<strong>{session.displayName}</strong></div><nav className="pc2-nav">{views.map(v=><button key={v} className={view===v?'active':''} onClick={()=>{setView(v);setSelectedJob(null);setSelectedJobDetails(null);setSelectedBlock(null);setAllocation(null)}}>{v}</button>)}</nav><button className="pc2-logout" onClick={logout}>Logout session</button></aside>
    <main className="pc2-main"><header className="pc2-top"><div><h1>{view}</h1><p>Planner control centre · Live operational data</p></div><div className="pc2-actions"><select value={department} onChange={e=>setDepartment(e.target.value)}>{departments.map(d=><option key={d}>{d}</option>)}</select><button onClick={load}>Refresh</button><button className="primary" disabled={busy} onClick={optimize}>{busy?'Optimizing…':'Run Optimizer'}</button></div></header>
      <div className="pc2-content">
        ...
        {view==='Block Allocation'&&<><section className="pc2-card"><div className="pc2-cardhead"><div><h3>Block allocation &amp; safety slot management</h3><p className="sub">Select a maintenance job and a block. MARS checks section, restriction, isolation and train-free feasibility before any plan change.</p></div></div><div className="pc2-toolbar"><select value={selectedJob?.job_id||''} onChange={e=>{setSelectedJob(jobs.find(j=>String(j.job_id)===e.target.value)||null);setSelectedJobDetails(null);setAllocation(null)}}><option value="">Select maintenance job…</option>{filteredJobs.map(j=><option key={j.job_id} value={j.job_id}>{j.job_id} · {j.department} · {sectionForJob(j)||'—'}</option>)}</select><button className="pc2-primary" disabled={!selectedJob||!selectedBlock||busy} onClick={check}>{busy?'Checking…':'Check selected allocation'}</button></div><div className="pc2-blockgrid">{blocks.map(b=><button key={b.block_id} className={`pc2-block ${selectedBlock?.block_id===b.block_id?'selected':''} ${conflicts.some(c=>String(c.block_id)===String(b.block_id))?'conflict':''}`} onClick={()=>{setSelectedBlock(b);setAllocation(null)}}><h4>{b.block_id}</h4><p>{fmt(b.section_id)} · {fmt(b.status)}</p><p className="window">{fmt(b.block_date)} · {fmt(b.start_time)}–{fmt(b.end_time)}</p><p>{fmt(b.block_type)} · {fmt(b.restrictions,'No restriction')}</p><div className="util"><span>{fmt(b.assigned_job_count,0)} assigned</span><strong>{fmt(b.utilization,0)}%</strong></div></button>)}</div>{selectedJob&&selectedBlock&&<div className="pc2-message info"><strong>Allocation selection:</strong> {fmt(selectedJob.job_id)} → {fmt(selectedBlock.block_id)} · {fmt(selectedBlock.section_id)} · {fmt(selectedBlock.block_date)} · {fmt(selectedBlock.start_time)}–{fmt(selectedBlock.end_time)}. Run the allocation check to validate this combination.</div>}{selectedBlock&&!selectedJob&&<div className="pc2-message info"><strong>Block selected:</strong> {fmt(selectedBlock.block_id)} · {fmt(selectedBlock.section_id)} · {fmt(selectedBlock.block_date)} · {fmt(selectedBlock.start_time)}–{fmt(selectedBlock.end_time)}. Now choose a maintenance job.</div>}{selectedJob&&!selectedBlock&&<div className="pc2-message info"><strong>Job selected:</strong> {fmt(selectedJob.job_id)} · {fmt(selectedJob.department)} · {fmt(sectionForJob(selectedJob))}. Now choose a block.</div>}{allocation&&<div className={`pc2-check ${allocation.feasible?'ok':'bad'}`}><strong>{allocation.feasible?'✓ Feasible allocation':'✕ Allocation blocked'}</strong><div>{fmt(allocation.reason)}</div>{allocation.slots?.length>0&&<small>Candidate windows: {allocation.slots.map((s:any)=>`${new Date(s.start).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}–${new Date(s.end).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}`).join(', ')}</small>}</div>}</section></>}

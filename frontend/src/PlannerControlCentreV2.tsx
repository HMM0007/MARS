...
export default function PlannerControlCentreV2({ session, onLogout }: Props) {
  ...
  const sectionForJob = (job: Job | null) => planForJob(job)?.section_id || job?.section_id || job?.section || ''
  const blockForJob = (job: Job | null) => planForJob(job)?.block_id || job?.block_id || job?.block || ''
  ...
  return <div className="pc2-shell">
    <aside className="pc2-sidebar"><div className="pc2-brand"><div className="pc2-rail">रेल</div><div><strong>MARS</strong><small>Maintenance Allocation &amp; Routing System</small></div></div><div className="pc2-role">DIVISIONAL PLANNER<strong>{session.displayName}</strong></div><nav className="pc2-nav">{views.map(v=><button key={v} className={view===v?'active':''} onClick={()=>{setView(v);setSelectedJob(null);setAllocation(null)}}>{v}</button>)}</nav><button className="pc2-logout" onClick={logout}>Logout session</button></aside>
    <main className="pc2-main"><header className="pc2-top"><div><h1>{view}</h1><p>Planner control centre · Live operational data</p></div><div className="pc2-actions"><select value={department} onChange={e=>setDepartment(e.target.value)}>{departments.map(d=><option key={d}>{d}</option>)}</select><button onClick={load}>Refresh</button><button className="primary" disabled={busy} onClick={optimize}>{busy?'Optimizing…':'Run Optimizer'}</button></div></header>
      <div className="pc2-content">
        ...
        {view==='Control Centre'&&<>
          ...
          <div className="pc2-grid"><section className="pc2-card"><div className="pc2-cardhead"><div><h3>Requests needing planning</h3><p className="sub">All departments are visible to the divisional planner.</p></div><button onClick={()=>setView('Requests')}>View all →</button></div><div className="pc2-depts"><button className={department==='All'?'active':''} onClick={()=>setDepartment('All')}>All · {jobs.length}</button>{departmentsCount.map(x=><button key={x.d} className={department===x.d?'active':''} onClick={()=>setDepartment(x.d)}>{x.d} · {x.n}</button>)}</div><div className="pc2-table"><table><thead><tr><th>Job</th><th>Department</th><th>Section</th><th>Block</th><th>Priority</th></tr></thead><tbody>{filteredJobs.slice(0,8).map(j=><tr className="clickable" key={j.job_id} onClick={()=>setSelectedJob(j)}><td><strong>{fmt(j.job_id)}</strong></td><td>{fmt(j.department)}</td><td>{fmt(sectionForJob(j))}</td><td>{fmt(blockForJob(j))}</td><td><span className={`pc2-tag ${tone(j.priority)}`}>{fmt(j.priority)}</span></td></tr>)}</tbody></table></div></section><section className="pc2-card"><div className="pc2-cardhead"><div><h3>Operational attention</h3><p className="sub">Only backend-detected issues are shown here.</p></div><button onClick={()=>setView('Alerts')}>View all →</button></div>{conflicts.slice(0,5).map((c,i)=><div className="pc2-risk" key={c.conflict_id||i}><span className="pc2-risk-dot"/><div><strong>{fmt(c.description,c.type)}</strong><small>Job: {fmt(c.job_ids?.join(', '))} · Section: {fmt(c.section_id)} · Block: {fmt(c.block_id)} · {fmt(c.severity,'INFO')}</small></div></div>)}{!conflicts.length&&<div className="pc2-empty">No active conflicts detected.</div>}</section></div>
        </>}
        {view==='Requests'&&<section className="pc2-card"><div className="pc2-cardhead"><div><h3>Maintenance request catalogue</h3><p className="sub">Every request includes its asset-derived section and current-plan allocation.</p></div></div><div className="pc2-toolbar"><input placeholder="Search job, department, section, block…" value={query} onChange={e=>setQuery(e.target.value)}/><select value={department} onChange={e=>setDepartment(e.target.value)}>{departments.map(d=><option key={d}>{d}</option>)}</select></div><div className="pc2-table"><table><thead><tr><th>Job</th><th>Work</th><th>Department</th><th>Section</th><th>Block</th><th>Duration</th><th>Priority</th><th>Plan</th></tr></thead><tbody>{filteredJobs.map(j=><tr className="clickable" key={j.job_id} onClick={()=>setSelectedJob(j)}><td><strong>{fmt(j.job_id)}</strong></td><td>{fmt(j.work_type||j.description)}</td><td>{fmt(j.department)}</td><td>{fmt(sectionForJob(j))}</td><td>{fmt(blockForJob(j))}</td><td>{fmt(j.duration_min)} min</td><td><span className={`pc2-tag ${tone(j.priority)}`}>{fmt(j.priority)}</span></td><td><span className={`pc2-tag ${tone(j.plan_status)}`}>{fmt(j.plan_status,'UNPLANNED')}</span></td></tr>)}</tbody></table></div></section>}
        {view==='Block Allocation'&&<><section className="pc2-card"><div className="pc2-cardhead"><div><h3>Block allocation &amp; safety slot management</h3><p className="sub">Select a maintenance job and a block. MARS checks section, restriction, isolation and train-free feasibility before any plan change.</p></div></div><div className="pc2-toolbar"><select value={selectedJob?.job_id||''} onChange={e=>{setSelectedJob(jobs.find(j=>String(j.job_id)===e.target.value)||null);setSelectedBlock(null);setAllocation(null)}}><option value="">Select maintenance job…</option>{filteredJobs.map(j=><option key={j.job_id} value={j.job_id}>{j.job_id} · {j.department} · {sectionForJob(j)||'—'}</option>)}</select><button className="pc2-primary" disabled={!selectedJob||!selectedBlock||busy} onClick={check}>{busy?'Checking…':'Check selected allocation'}</button></div><div className="pc2-blockgrid">{blocks.map(b=><button key={b.block_id} className={`pc2-block ${selectedBlock?.block_id===b.block_id?'selected':''} ${conflicts.some(c=>String(c.block_id)===String(b.block_id))?'conflict':''}`} onClick={()=>{setSelectedJob(null);setSelectedBlock(b);setAllocation(null)}}><h4>{b.block_id}</h4><p>{fmt(b.section_id)} · {fmt(b.status)}</p><p className="window">{fmt(b.block_date)} · {fmt(b.start_time)}–{fmt(b.end_time)}</p><p>{fmt(b.block_type)} · {fmt(b.restrictions,'No restriction')}</p><div className="util"><span>{fmt(b.assigned_job_count,0)} assigned</span><strong>{fmt(b.utilization,0)}%</strong></div></button>)}</div>{selectedBlock&&<div className="pc2-message info"><strong>Block selected:</strong> {fmt(selectedBlock.block_id)} · {fmt(selectedBlock.section_id)} · {fmt(selectedBlock.block_date)} · {fmt(selectedBlock.start_time)}–{fmt(selectedBlock.end_time)}. Now choose a maintenance job and run the allocation check.</div>}{allocation&&<div className={`pc2-check ${allocation.feasible?'ok':'bad'}`}><strong>{allocation.feasible?'✓ Feasible allocation':'✕ Allocation blocked'}</strong><div>{fmt(allocation.reason)}</div>{allocation.slots?.length>0&&<small>Candidate windows: {allocation.slots.map((s:any)=>`${new Date(s.start).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}–${new Date(s.end).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}`).join(', ')}</small>}</div>}</section></>}
    ...
    {selectedJob&&<div className="pc2-modal-backdrop" onClick={()=>setSelectedJob(null)}><div className="pc2-modal" onClick={e=>e.stopPropagation()}><h3>{fmt(selectedJob.job_id)}</h3><p>{fmt(selectedJob.work_type||selectedJob.description)}</p><div className="pc2-planbar"><span>Department <b>{fmt(selectedJob.department)}</b></span><span>Section <b>{fmt(sectionForJob(selectedJob))}</b></span><span>Block <b>{fmt(blockForJob(selectedJob))}</b></span><span>Priority <b>{fmt(selectedJob.priority)}</b></span></div><div className="pc2-modal-actions"><button onClick={()=>setSelectedJob(null)}>Close</button></div></div></div>}
  </div>
}

...
function AppContent() {
  const location = useLocation(); const navigate = useNavigate(); const isLoginPage = location.pathname === '/login';
  ...
  return <div className="min-h-screen bg-[#EEF2F6] font-sans text-[#17345C]"><Header selectedDivision={selectedDivision} selectedRole={selectedRole} onDivisionChange={setSelectedDivision} onRoleChange={handleRoleChange} /><OperationalStrip planData={weeklyPlan} monthlyData={monthlyPlan} selectedDivision={selectedDivision.name} /><OperationalWorkflowBridge /><div className="flex min-h-[calc(100vh-118px)]"><Sidebar currentRole={selectedRole}/><div className="min-w-0 flex-1 overflow-y-auto"><Routes>
    <Route path="/" element={<MARSHomePage currentRole={selectedRole}/>}/><Route path="/command-center" element={<PlannerCommandCenterPage currentRole={selectedRole}/>}/><Route path="/corridor" element={<CorridorMapPage currentRole={selectedRole}/>}/><Route path="/what-if" element={<WhatIfScenarioPage currentRole={selectedRole}/>}/><Route path="/weekly" element={<WeeklyPlanPage currentRole={selectedRole}/>}/><Route path="/monthly" element={<MonthlyPlanPage currentRole={selectedRole}/>}/><Route path="/jobs" element={<JobsPage currentRole={selectedRole}/>}/><Route path="/jobs/priority-risk" element={<PriorityRiskPage currentRole={selectedRole}/>}/><Route path="/jobs/department/engineering" element={<DepartmentWorkCentrePage department="Engineering" currentRole={selectedRole}/>}/><Route path="/jobs/department/snt" element={<DepartmentWorkCentrePage department="S&T" currentRole={selectedRole}/>}/><Route path="/jobs/department/traction" element={<DepartmentWorkCentrePage department="Traction" currentRole={selectedRole}/>}/><Route path="/jobs/department" element={<DepartmentWorkCentrePage department="Engineering" currentRole={selectedRole}/>}/><Route path="/dept/engineering" element={<EngineeringDashboardPage currentRole={selectedRole}/>}/><Route path="/dept/engineering/*" element={<EngineeringDashboardPage currentRole={selectedRole}/>}/><Route path="/dept/snt" element={<SNTDashboardPage currentRole={selectedRole}/>}/><Route path="/dept/snt/*" element={<SNTDashboardPage currentRole={selectedRole}/>}/><Route path="/dept/traction" element={<TractionDashboardPage currentRole={selectedRole}/>}/><Route path="/dept/traction/*" element={<TractionDashboardPage currentRole={selectedRole}/>}/><Route path="/impact" element={<ImpactReportsPage currentRole={selectedRole}/>}/><Route path="/integration" element={<IntegrationStatusPage currentRole={selectedRole}/>}/>
  </Routes><footer className="border-t border-[#D6DEE6] bg-white px-5 py-2"><div className="flex flex-col justify-between gap-1 text-[10px] text-[#66788A] sm:flex-row"><div><strong className="text-[#123C70]">MARS 2.0</strong> &nbsp;|&nbsp; Ministry of Railways, Government of India &nbsp;•&nbsp; Central Railway (Pune Division)</div><div className="font-mono font-bold">PS 26027 &nbsp;•&nbsp; <span className="text-[#16865F]">RAILWAY RULES AUDITED</span> &nbsp;•&nbsp; ROLE: {selectedRole?.name}</div></div></footer></div></div></div>;
}

...
export default function Header({ selectedDivision, selectedRole, onDivisionChange, onRoleChange }) {
  ...
  return (
    <header className="sticky top-0 z-50 text-white shadow-[0_2px_12px_rgba(0,36,86,.24)]">
      <div className="relative flex min-h-[74px] w-full items-center border-b border-white/20 bg-gradient-to-r from-[#0755B4] via-[#1769D4] to-[#0755B4] px-4 lg:px-6">
        <div className="pointer-events-none absolute inset-0 opacity-[.06] bg-[linear-gradient(90deg,transparent_0,transparent_49%,#fff_50%,transparent_51%,transparent_100%)] bg-[length:80px_80px]" />

        <div className="relative flex w-[31%] min-w-0 items-center gap-3">
          ...
          <Link to="/" className="flex min-w-0 items-center gap-3">
            <div className="flex h-[56px] w-[56px] shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-white/90 bg-white shadow-lg">
              <BrandAsset name="indian_railways_logo.png" alt="Indian Railways" className="h-full w-full object-contain p-1" fallback={<span className="text-[10px] font-black text-[#0757B8]">IR</span>} />
            </div>
        ...
        <div className="relative flex min-w-0 flex-1 justify-center text-center">
          <Link to="/" className="flex min-w-0 items-center gap-3">
            <div className="hidden h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/40 bg-white/10 lg:flex">
            ...
            <div className="min-w-0 text-left leading-none">
              <div className="text-[34px] font-black tracking-[-.03em] sm:text-[39px]">MARS 2.0</div>
              <div className="mt-1 text-[9px] font-semibold tracking-[.18em] text-white/85 sm:text-[11px]">MAINTENANCE &amp; ASSET RESOURCE SCHEDULING</div>
            </div>
          </Link>
        </div>

        <div className="relative flex w-[31%] items-center justify-end gap-2 lg:gap-3">
          ...
          <button className="relative rounded-full p-2 hover:bg-white/15" title="Operational alerts" aria-label="Operational alerts">
            <Bell className="h-5 w-5" />
            <span className="absolute right-1 top-1 flex h-2.5 min-w-2.5 items-center justify-center rounded-full bg-[#E31B23] ring-2 ring-[#1769D4]" />
          </button>
          <div className="relative">
            <button type="button" onClick={() => { setRoleOpen(!roleOpen); setDivisionOpen(false); }} className="flex items-center gap-2 rounded-full px-1 py-1 hover:bg-white/15">
              ...
              <span className="hidden max-w-[175px] text-left lg:block"><span className="block truncate text-[11px] font-bold">{role.shortName || role.name}</span><span className="block truncate text-[9px] text-white/75">{role.dept} • Pune Division (CR)</span></span>
              <ChevronDown className="hidden h-4 w-4 lg:block" />
            </button>
            {roleOpen && <Dropdown className="right-0 w-72">
      ...
      <div className="flex h-[31px] items-center justify-between overflow-x-auto bg-white px-4 text-[10px] text-[#17345C] shadow-sm md:px-5">

...
import { Calendar, ChevronLeft, ChevronRight, Filter, RefreshCw, ShieldCheck, Train, X, ZoomIn, ZoomOut } from 'lucide-react';
...
// A possession is shared only when the weekly solver actually attached jobs
// from two or more departments. is_consolidated alone is not sufficient.
const getDepartments = (block) => Array.isArray(block?.departments) ? block.departments.filter(Boolean) : [];
const isSharedBlock = (block) => getDepartments(block).length > 1;
const deptForBlock = (block) => {
  const depts = getDepartments(block);
  return isSharedBlock(block) ? 'Consolidated' : (depts[0] || 'Engineering');
};
...
export default function UnifiedGantt({
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [plan, timetable] = await Promise.all([fetchWeeklyPlan(), fetchCOATimetable().catch(() => [])]);
  ...
  const filteredBlocks = useMemo(() => blocks.filter((block) => {
    ...
    const deptMatch = selectedDept === 'ALL' || dept === selectedDept || (selectedDept === 'Shared' && isSharedBlock(block));
  ...
  // Selecting a block must stay bound to that exact block. Do not perform a
  // section lookup after the click; doing so can replace the selected block
  // with the first possession in the same section.
  const handleBlock = useCallback((block) => {
    setSelectedId(block.block_id);
    onBlockClick?.(block);
    onSelectBlock?.(block);
    onOpenExplainability?.(block);
  }, [onBlockClick, onSelectBlock, onOpenExplainability]);
    ...
    <section className="overflow-hidden rounded-lg border border-[#C9D4DF] bg-white shadow-sm" aria-label="Weekly possession schedule">
      ...
      <div className="flex min-h-[610px] max-h-[720px] overflow-hidden">
        ...
        <div ref={timelineRef} className="relative flex-1 overflow-auto bg-white"><div style={{ width: timelineWidth, minWidth: '100%' }}>
          ...
          <div className="relative"><div className="absolute inset-0 pointer-events-none">{days.map((day, i) => <div key={day.toISOString()} style={{ left: i * dayWidth, width: dayWidth }} className={`absolute top-0 bottom-0 border-r border-[#D6DEE6] ${i % 2 ? 'bg-[#FBFCFD]' : 'bg-white'}`}>{[1,2,3,4,5].map((n) => <span key={n} style={{ left: `${(n / 6) * 100}%` }} className="absolute top-0 bottom-0 border-l border-[#F0F3F6]" />)}</div>)}</div>
            ...
            {filteredTracks.map((track) => {
              ...
              return <div key={track} style={{ height: rowHeight }} className="relative border-b border-[#E2E8F0]">
                ...
                {trackBlocks.map((block) => {
                  ...
                  const dept = shared ? 'Consolidated' : deptForBlock(block);
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-[#D6DEE6] bg-[#F8FAFC] px-4 py-2.5"><div className="flex flex-wrap items-center gap-3 text-[9px] font-semibold text-[#52606D]"><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm" style={{ background: COLORS.Engineering }} />Engineering</span><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm" style={{ background: COLORS['S&T'] }} />S&T</span><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm" style={{ background: COLORS.Traction }} />Traction</span><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm" style={{ background: COLORS.Consolidated }} />Shared possession</span><span className="flex items-center gap-1"><Train className="h-3 w-3 text-[#8F1D2C]" />Train movement</span><span className="text-[#7B8794]">Red dot on a possession = critical priority</span></div>{selectedBlock && <div className="flex items-center gap-2 rounded border border-[#C9D4DF] bg-white px-2.5 py-1.5"><ShieldCheck className="h-3.5 w-3.5 text-[#2F9E44]" /><span className="font-mono text-[9px] font-bold text-[#1E3A5F]">{selectedBlock.block_id}</span><span className="text-[9px] text-[#52606D]">{formatTime(parseDate(selectedBlock.start_time))}–{formatTime(parseDate(selectedBlock.end_time))}</span><button onClick={() => setSelectedId(null)} className="text-[#7B8794] hover:text-[#1F2933]"><X className="h-3 w-3" /></button></div>}</div>
    </section>
  );
}

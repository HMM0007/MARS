...
function possessionFeature(block) {
  ...
  return {
    type: 'Feature',
    properties: { entity_type: 'BLOCK', block_id: block.block_id || 'BLOCK', color: status === 'DEFERRED' ? COLORS.Deferred : colorOf(departments), status, department: departments.join(' + '), job_count: details.length, track_id: trackId, location_km: anchor, start_km: anchor - spread, end_km: anchor + spread },
...
function jobFeature(job) {
  ...
  return {
    type: 'Feature',
    properties: { entity_type: 'JOB', job_id: job.job_id || 'JOB', color: status === 'DEFERRED' ? COLORS.Deferred : COLORS.Pending, status, department: departments.join(' + '), location_km: km, track_id: trackId, defect_type: job.defect_type || job.maintenance_type || 'Maintenance', asset_id: job.asset_id || '', criticality: job.criticality_level || job.criticality || '' },
    geometry: { type: 'LineString', coordinates: [getCoordinatesForKm(km - 0.18, trackId), getCoordinatesForKm(km + 0.18, trackId)] },
  };
}
...
export default function SatelliteMap({ blocks = [], jobs = [], onOpenExplainability, isFullScreenMode = false, onToggleFullScreen }) {
  ...
  const counts = useMemo(() => ({
    ...
    scheduled: blocks.filter((b) => b.status !== 'DEFERRED').length,
    deferred: blocks.filter((b) => b.status === 'DEFERRED').length,
    pending: jobs.filter((j) => !scheduledJobIds.has(j.job_id) && j.status !== 'DEFERRED').length,
    critical: jobs.filter((j) => (j.criticality_level || j.criticality) === 'CRITICAL').length,
  }), [blocks, jobs, scheduledJobIds]);
  ...
  const updateStationLabels = () => {
    ...
    stationLabelRefs.current.forEach(({ el, station }) => {
      ...
      el.style.transform = `translate(${p.x}px,${p.y - 14}px) translate(-50%,-100%)`;
    });
  };
  ...
  const updateActivityLabels = () => {
    ...
    activityFeatures.forEach((feature, index) => {
      ...
      el.type = 'button';
      el.textContent = isBlock ? `${feature.properties.block_id}  •  ${feature.properties.job_count || 0} JOBS` : feature.properties.job_id;
      el.title = isBlock ? `${feature.properties.block_id} • ${feature.properties.status} • ${feature.properties.start_km?.toFixed?.(2) || ''}-${feature.properties.end_km?.toFixed?.(2) || ''} km` : `${feature.properties.job_id} • ${feature.properties.defect_type} • Km ${Number(feature.properties.location_km).toFixed(2)}`;
      const offset = (index % 3 - 1) * 18;
      el.style.cssText = `position:absolute;left:0;top:0;z-index:18;pointer-events:auto;transform:translate(${p.x + offset}px,${p.y - (isBlock ? 20 : 15)}px) translate(-50%,-50%);background:${isBlock ? feature.properties.color : '#FFF7ED'};color:${isBlock ? '#FFFFFF' : '#9A3412'};border:2px solid ${feature.properties.color};border-radius:4px;padding:3px 6px;font:800 ${isBlock ? 8 : 7}px/11px Arial,sans-serif;box-shadow:0 2px 5px rgba(0,0,0,.24);white-space:nowrap;cursor:pointer;${critical ? 'box-shadow:0 0 0 2px #B42318,0 2px 6px rgba(0,0,0,.25);' : ''}`;
  ...
  useEffect(() => {
    ...
    const source = map.getSource('operational-activity');
  ...
  useEffect(() => {
    ...
    const addOperationalLayers = () => {
      ...
      const inspect = (e) => {
        const features = map.queryRenderedFeatures(e.point, { layers: ['possession-block', 'job-marker-line', 'job-core'] });
        if (features[0]) setInspection({ type: 'ACTIVITY', data: features[0].properties });
      };
      map.on('click', inspect);
      map.on('move', refreshLabels); map.on('zoom', refreshLabels); map.on('resize', refreshLabels);
      map.fitBounds(CORRIDOR_BOUNDS, { padding: { top: 100, right: 110, bottom: 95, left: 95 }, duration: 0 });
    ...
    map.once('load', () => {
      try { addOperationalLayers(); } catch (error) { console.error('Operational railway layers failed:', error); setMapError(error?.message || 'Railway control-centre layers could not be initialized.'); setLoaded(true); }
    });
  ...
  const shell = <div className={`relative overflow-hidden border border-[#C8D2DC] bg-[#DCE5E9] shadow-sm ${isFullScreenMode ? 'fixed inset-0 z-[2147483000] h-screen w-screen rounded-none' : 'h-[calc(100vh-245px)] min-h-[620px] rounded-lg'}`}>
    ...
    <div className="pointer-events-none absolute left-4 top-4 z-30 rounded-md border border-[#BFCBD6] bg-[#F8FAFC]/96 px-3 py-2 shadow-lg">
      <div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.16em] text-[#52606D]"><Activity className="h-3.5 w-3.5 text-[#173E6C]" /> Central Railway • Operations Control</div>
      <div className="text-sm font-extrabold text-[#102A43]">Pune–Lonavala Double Line</div>
      <div className="text-[10px] font-mono text-[#52606D]">Km 191.00 → 254.84 • UP / DN • 25 kV AC</div>
    </div>
    ...
    {inspection && <aside className="absolute bottom-20 right-4 z-50 w-[350px] rounded-lg border border-[#C7D1DA] bg-white p-3 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><div className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#7B8794]">Maintenance Activity</div><div className="text-sm font-extrabold text-[#173E6C]">{inspection.data.block_id || inspection.data.job_id}</div></div><button onClick={() => setInspection(null)} className="rounded p-1 text-[#718294] hover:bg-[#F4F6F8]"><X className="h-4 w-4" /></button></div><div className="mt-3 space-y-2 text-[10px]"><div className="grid grid-cols-2 gap-2"><div className="rounded bg-[#F4F6F8] p-2"><div className="text-[#8796A5]">Department</div><div className="font-bold">{inspection.data.department || 'Maintenance'}</div></div><div className="rounded bg-[#F4F6F8] p-2"><div className="text-[#8796A5]">Status</div><div className="font-bold">{inspection.data.status || 'PENDING'}</div></div></div><div className="grid grid-cols-2 gap-2"><div className="rounded bg-[#F4F6F8] p-2"><div className="text-[#F4F6F8] p-2"><div className="text-[#8796A5]">Chainage</div><div className="font-mono font-bold">{Number(inspection.data.location_km || 0).toFixed(2)} km</div></div><div className="rounded bg-[#F4F6F8] p-2"><div className="text-[#8796A5]">Track</div><div className="font-mono font-bold">{inspection.data.track_id || 'UP'}</div></div></div><div className="rounded bg-[#FFF7ED] p-2"><div className="text-[#9A3412]">Work / Defect</div><div className="font-bold text-[#7C2D12]">{inspection.data.defect_type || `${inspection.data.job_count || 0} jobs in possession`}</div></div><button onClick={() => onOpenExplainability?.(inspection.data)} className="w-full rounded bg-[#173E6C] px-3 py-2 text-[10px] font-bold text-white">Open Block Details</button></div></aside>}

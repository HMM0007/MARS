...
function possessionFeature(block) {
  ...
  return {
    type: 'Feature',
    properties: { entity_type: 'BLOCK', block_id: block.block_id, color: status === 'DEFERRED' ? COLORS.Deferred : colorOf(departments), status, department: departments.join(' + '), job_count: details.length },
...
function jobFeature(job) {
  ...
  const half = Math.max(0.10, Math.min(0.30, Number(job.duration_hours || job.duration || 1) * 0.08));
  const trackId = job.track_id || 'PUNE-LNL-UP';
  return {
    type: 'Feature',
    properties: { entity_type: 'JOB', job_id: job.job_id, color: status === 'DEFERRED' ? COLORS.Deferred : COLORS.Pending, status, department: departments.join(' + '), location_km: km, defect_type: job.defect_type || job.maintenance_type || 'Maintenance' },
    geometry: { type: 'LineString', coordinates: [getCoordinatesForKm(km - half, trackId), getCoordinatesForKm(km + half, trackId)] },
  };
}
...
export default function SatelliteMap({ blocks = [], jobs = [], onOpenExplainability, isFullScreenMode = false, onToggleFullScreen }) {
  ...
  useEffect(() => {
    ...
    const renderOperationalOverlay = () => {
      ...
      // Railway alignment: white casing keeps the two operational tracks readable over OSM.
      railwayRef.current.forEach((feature) => {
        const trackColor = feature.properties?.track_id === 'PUNE-LNL-DN' ? '#2F6F7E' : '#173E6C';
        addPolyline(feature.geometry.coordinates, '#FFFFFF', 9, 0.96);
        addPolyline(feature.geometry.coordinates, trackColor, 4, 1);
      });

      // Maintenance is drawn as a possession directly over the affected railway alignment.
      activityFeaturesRef.current.forEach((feature) => {
        const p = feature.geometry.coordinates;
        const c = feature.properties.color;
        const dash = feature.properties.status === 'PENDING' ? '5 5' : feature.properties.status === 'DEFERRED' ? '3 5' : '';
        addPolyline(p, '#FFFFFF', feature.properties.status === 'SCHEDULED' ? 13 : 10, 0.94);
        addPolyline(p, c, feature.properties.status === 'SCHEDULED' ? 8 : 5, 1, dash);
      });

      // Fixed station markers sit on top of the track, so the corridor can be read at a glance.
      PUNE_LNL_STATIONS.forEach((station) => {
        ...
        halo.setAttribute('stroke', '#173E6C');
    ...
    const updateStationLabels = () => stationLabelRefs.current.forEach(({ el, station }) => {
      const p = map.project([station.lng, station.lat]);
      el.style.transform = `translate(${p.x}px,${p.y + 9}px) translate(-50%,0)`;
    });

    const updateActivityLabels = () => {
      activityLabelRefs.current.forEach(({ el }) => el.remove());
      activityLabelRefs.current = [];
      if (map.getZoom() < 11.7) return;
      activityFeaturesRef.current.forEach((feature) => {
        const coords = feature.geometry.coordinates;
        const p = map.project(coords[Math.floor(coords.length / 2)]);
        const el = document.createElement('button');
        el.type = 'button';
        el.textContent = feature.properties.entity_type === 'BLOCK' ? feature.properties.block_id : feature.properties.job_id;
        el.style.cssText = `position:absolute;left:0;top:0;z-index:8;pointer-events:auto;transform:translate(${p.x}px,${p.y - 16}px) translate(-50%,-50%);background:#fff;border:1px solid ${feature.properties.color};border-radius:3px;padding:2px 4px;font:700 8px/11px Arial;color:#1F2933;box-shadow:0 1px 3px rgba(0,0,0,.18);white-space:nowrap;`;
    ...
    map.once('load', async () => {
      try {
        ...
        overlay.style.cssText = 'position:absolute;left:0;top:0;width:100%;height:100%;z-index:4;pointer-events:none;overflow:visible;';
        ...
        try {
          ...
          if (response.ok) {
            ...
            const lines = railway?.features?.filter((f) => f.geometry?.type === 'LineString') || [];
            if (lines.length) railwayRef.current = lines;
          }
        } catch (error) {
          console.warn('Detailed railway geometry unavailable; fixed corridor alignment retained.', error);
        }
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => renderOverlayRef.current?.());
  }, [activityFeatures]);
  ...
  useEffect(() => {
    ...
    const a = setTimeout(refresh, 150);
    const b = setTimeout(refresh, 500);

...
function makeBlockFeature(block, jobsById, scheduledIds) {
  ...
  const spread = Math.max(0.18, Math.min(1.2, (maxKm - minKm) / 2 + 0.10));
  ...
  return {
    ...
    geometry: {
      type: 'LineString',
      coordinates: [getCoordinatesForKm(minKm, trackId), getCoordinatesForKm(maxKm, trackId)],
    },
  };
}
...
function makeJobFeature(job, scheduledIds) {
  ...
  return {
    ...
    geometry: {
      type: 'LineString',
      coordinates: [getCoordinatesForKm(km - 0.20, trackId), getCoordinatesForKm(km + 0.20, trackId)],
    },
  };
}
...
export default function SatelliteMap({ blocks = [], jobs = [], onOpenExplainability, isFullScreenMode = false, onToggleFullScreen }) {
  ...
  const activityFeatures = useMemo(() => {
    const blockFeatures = blocks.map((block) => makeBlockFeature(block, jobsById, scheduledJobIds)).filter(Boolean).filter((feature) => {
      const departments = feature.properties.department ? feature.properties.department.split(' + ') : [];
      if (dept !== 'ALL' && dept !== 'Shared' && !departments.includes(dept)) return false;
      if (dept === 'Shared' && departments.length < 2) return false;
      if (status !== 'ALL' && status !== feature.properties.status) return false;
      const p = feature.properties;
      const text = `${p.block_id} ${p.job_ids.join(' ')} ${p.department} ${p.track_id} ${p.start_km} ${p.end_km}`.toLowerCase();
      return !query || text.includes(query.toLowerCase());
    });

    const jobFeatures = jobs.map((job) => makeJobFeature(job, scheduledJobIds)).filter(Boolean).filter((feature) => {
      const departments = feature.properties.department ? feature.properties.department.split(' + ') : [];
      if (dept !== 'ALL' && dept !== 'Shared' && !departments.includes(dept)) return false;
      if (dept === 'Shared' && departments.length < 2) return false;
      if (status !== 'ALL' && status !== feature.properties.status) return false;
      const p = feature.properties;
      const text = `${p.job_id} ${p.asset_id} ${p.asset_type} ${p.section_id} ${p.track_id} ${p.defect_type} ${p.location_km}`.toLowerCase();
      return !query || text.includes(query.toLowerCase());
    });
  ...
  const counts = useMemo(() => ({
    ...
    scheduled: jobs.filter((j) => scheduledJobIds.has(jobKey(j?.job_id))).length,
    pending: jobs.filter((j) => !scheduledJobIds.has(jobKey(j?.job_id)) && j?.status !== 'DEFERRED' && j?.status !== 'COMPLETED').length,
    deferred: jobs.filter((j) => j?.status === 'DEFERRED').length,
    critical: jobs.filter((j) => (j?.criticality_level || j?.criticality) === 'CRITICAL').length,
  }), [jobs, blocks, scheduledJobIds]);

  const clearActivityLabels = () => {
    activityLabelRefs.current.forEach(({ el }) => el.remove());
    activityLabelRefs.current = [];
  };
  ...
  const updateStationLabels = () => {
    ...
    stationLabelRefs.current.forEach(({ el, station }) => {
      ...
      const visible = map.getBounds().contains([station.lng, station.lat]);
      el.style.display = visible ? 'block' : 'none';
      el.style.transform = `translate(${p.x}px,${p.y - 10}px) translate(-50%,-100%)`;
    });
  };
  ...
  const updateActivityLabels = () => {
    ...
    activityFeaturesRef.current.forEach((feature, index) => {
      const pData = feature.properties;
      const coords = feature.geometry.coordinates;
      const mid = coords[Math.floor(coords.length / 2)];
      const projected = map.project(mid);
      const isBlock = pData.entity_type === 'BLOCK';
      const critical = !isBlock && pData.criticality === 'CRITICAL';
      const priorityVisible = isBlock || critical || zoom >= 11.3;
      if (!priorityVisible) return;
      const tooClose = occupied.some((q) => Math.abs(q.x - projected.x) < 46 && Math.abs(q.y - projected.y) < 18);
      if (tooClose && zoom < 12.4) return;
      occupied.push({ x: projected.x, y: projected.y });
      const el = document.createElement('button');
      el.type = 'button';
      el.textContent = isBlock ? `${pData.block_id} · ${pData.job_count} JOBS` : pData.job_id;
      el.title = isBlock ? `${pData.block_id} • ${pData.start_km.toFixed(2)}–${pData.end_km.toFixed(2)} km • ${pData.department || 'Maintenance'}` : `${pData.job_id} • ${pData.defect_type} • Km ${pData.location_km.toFixed(2)}`;
      const background = isBlock ? pData.color : (pData.status === 'SCHEDULED' ? '#F0F7FF' : pData.status === 'DEFERRED' ? '#F3F4F6' : '#FFF4E8');
      const foreground = isBlock ? '#FFFFFF' : (pData.status === 'SCHEDULED' ? '#12528A' : pData.status === 'DEFERRED' ? '#4B5563' : '#9A3412');
      const offset = (index % 3 - 1) * 13;
      el.style.cssText = `position:absolute;left:0;top:0;z-index:30;pointer-events:auto;transform:translate(${projected.x + offset}px,${projected.y - (isBlock ? 21 : 14)}px) translate(-50%,-50%);background:${background};color:${foreground};border:2px solid ${pData.color};border-radius:4px;padding:3px 6px;font:800 ${isBlock ? 8 : 7}px/11px Arial,sans-serif;box-shadow:0 2px 6px rgba(0,0,0,.26);white-space:nowrap;cursor:pointer;${critical ? 'box-shadow:0 0 0 2px #B42318,0 2px 7px rgba(0,0,0,.28);' : ''}`;
      el.addEventListener('click', () => setInspection({ type: 'ACTIVITY', data: pData }));
  ...
  useEffect(() => {
    ...
    const source = map.getSource('maintenance-activity');
    if (source) source.setData({ type: 'FeatureCollection', features: activityFeatures });
    requestAnimationFrame(updateActivityLabels);
  }, [activityFeatures]);
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: {
        version: 8,
        sources: {
          osm: { type: 'raster', tiles: ['https://tile.openstreetmap.org/{z}/{x}/{y}.png'], tileSize: 256, minzoom: 0, maxzoom: 19, attribution: '© OpenStreetMap contributors' },
        ...
        layers: [
          ...
          { id: 'block-line', type: 'line', source: 'maintenance', filter: ['==', ['get', 'entity_type'], 'BLOCK'], minzoom: 9.5, paint: { 'line-color': ['get', 'color'], 'line-width': ['interpolate', ['linear'], ['zoom'], 9.5, 5, 12, 8, 16, 12], 'line-opacity': 0.96, 'line-dasharray': [1, 0] } },
    ...
    const loadRailway = async () => {
      try {
        ...
        const corridorLines = lines.filter((f) => {
          const p = f.properties || {};
          return p.section_id === 'PUNE-LNL' || String(p.track_id || '').includes('PUNE-LNL');
        });
        const usable = corridorLines.length ? corridorLines : lines;
        if (!usable.length) throw new Error('No railway LineString geometry found');
        ...
        map.setLayoutProperty('railway-up', 'visibility', 'visible');
        map.setLayoutProperty('railway-dn', 'visibility', 'visible');
      } catch (error) {
        console.warn('Detailed railway geometry unavailable; using corridor geometry.', error);
        const fallback = [
        ...
        map.getSource('railway')?.setData({ type: 'FeatureCollection', features: fallback });
      }
    };
    ...
    map.once('load', async () => {
      try {
        ...
        stationLabelRefs.current = PUNE_LNL_STATIONS.map((station) => {
          ...
          el.title = `${station.name} • ${station.code} • Km ${station.km.toFixed(2)}`;
  ...
  const shell = <div className={`relative overflow-hidden border border-[#B9C6D1] bg-[#DDE5E9] shadow-sm ${isFullScreenMode ? 'fixed inset-0 z-[2147483000] h-screen w-screen rounded-none' : 'h-[calc(100vh-245px)] min-h-[620px] rounded-lg'}`}>
    ...
    <div className="pointer-events-none absolute left-4 top-4 z-40 rounded-md border border-[#C9D4DE] bg-white/96 px-3 py-2 shadow-lg">
    ...
    <div className="absolute left-4 top-[94px] z-40 flex max-w-[720px] flex-wrap gap-1 rounded-md border border-[#C9D4DE] bg-white/96 p-1.5 shadow-lg">
    ...
    <div className="absolute right-4 top-[94px] z-40 w-[270px] rounded-md border border-[#C9D4DE] bg-[#F8FAFC]/96 p-2 shadow-lg">
      <div className="mb-1.5 flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#52606D]"><Activity className="h-3 w-3 text-[#12805C]" /> LIVE OPERATIONAL LAYERS</div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[9px] font-bold text-[#52606D]"><span><i className="mr-1 inline-block h-2 w-7 rounded bg-[#123E73]" />UP track</span><span><i className="mr-1 inline-block h-2 w-7 rounded bg-[#087A86]" />DN track</span><span><i className="mr-1 inline-block h-2 w-7 rounded bg-[#1769AA]" />Possession</span><span><i className="mr-1 inline-block h-2 w-7 rounded border border-[#E45718] bg-[#FFF4E8]" />Pending job</span></div>

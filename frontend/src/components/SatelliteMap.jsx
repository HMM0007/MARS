...
function possessionFeature(block) {
  ...
  const spread = kms.length > 1 ? Math.max(0.12, Math.min(0.70, (Math.max(...kms) - Math.min(...kms)) / 2 + 0.10)) : 0.22;
  ...
  return { type: 'Feature', properties: { entity_type: 'BLOCK', block_id: block.block_id, color: status === 'DEFERRED' ? COLORS.Deferred : colorOf(departments), status, department: departments.join(' + '), job_count: details.length, track_id: trackId, location_km: anchor }, geometry: { type: 'LineString', coordinates: [getCoordinatesForKm(anchor - spread, trackId), getCoordinatesForKm(anchor + spread, trackId)] } };
}
...
function jobFeature(job) {
  ...
  return { type: 'Feature', properties: { entity_type: 'JOB', job_id: job.job_id, color: status === 'DEFERRED' ? COLORS.Deferred : COLORS.Pending, status, department: departments.join(' + '), location_km: km, track_id: trackId, defect_type: job.defect_type || job.maintenance_type || 'Maintenance', asset_id: job.asset_id || '', criticality: job.criticality_level || job.criticality || '' }, geometry: { type: 'LineString', coordinates: [getCoordinatesForKm(km - 0.16, trackId), getCoordinatesForKm(km + 0.16, trackId)] } };
}
...
export default function SatelliteMap({ blocks = [], jobs = [], onOpenExplainability, isFullScreenMode = false, onToggleFullScreen }) {
  ...
  const activityFeaturesRef = useRef([]);
  const renderOverlayRef = useRef(null);
  const railwayRef = useRef(fallbackRailwayGeoJSON().features);
  ...
  useEffect(() => { activityFeaturesRef.current = activityFeatures; renderOverlayRef.current?.(); }, [activityFeatures]);
  ...
  useEffect(() => {
    ...
    const renderOperationalOverlay = () => {
      ...
      if (!svg || !map.isStyleLoaded()) return;
      const width = map.getCanvasContainer().clientWidth;
      const height = map.getCanvasContainer().clientHeight;
      ...
      const pointsFor = (coords) => coords.map((c) => { const p = project(c); return `${p.x.toFixed(1)},${p.y.toFixed(1)}`; }).join(' ');
      const polyline = (coords, stroke, widthPx, opacity = 1, dash = '') => { if (!coords?.length) return; const el = document.createElementNS(ns, 'polyline'); el.setAttribute('points', pointsFor(coords)); el.setAttribute('fill', 'none'); el.setAttribute('stroke', stroke); el.setAttribute('stroke-width', String(widthPx)); el.setAttribute('stroke-linecap', 'round'); el.setAttribute('stroke-linejoin', 'round'); el.setAttribute('opacity', String(opacity)); if (dash) el.setAttribute('stroke-dasharray', dash); svg.appendChild(el); };
      const circle = (lng, lat, r, fill, stroke = 'none', sw = 0) => { const p = project([lng, lat]); const el = document.createElementNS(ns, 'circle'); el.setAttribute('cx', p.x); el.setAttribute('cy', p.y); el.setAttribute('r', String(r)); el.setAttribute('fill', fill); if (stroke !== 'none') { el.setAttribute('stroke', stroke); el.setAttribute('stroke-width', String(sw)); } svg.appendChild(el); };
      const line = (a, b, stroke, widthPx, dash = '') => polyline([a, b], stroke, widthPx, 1, dash);

      // Primary and secondary railway tracks.
      railwayRef.current.forEach((feature) => {
        const id = feature.properties?.track_id || '';
        const trackColor = id.includes('DN') ? '#2F6F7E' : '#173E6C';
        polyline(feature.geometry.coordinates, '#FFFFFF', 10, 0.94);
        polyline(feature.geometry.coordinates, trackColor, 4, 1);
      });

      // Engineering/S&T/Traction possessions and pending job locations.
      activityFeaturesRef.current.forEach((feature) => {
        const { status: itemStatus, color, entity_type: entityType } = feature.properties;
        const coords = feature.geometry.coordinates;
        if (entityType === 'BLOCK') {
          polyline(coords, '#FFFFFF', 15, 0.95);
          polyline(coords, color, 9, 0.98, itemStatus === 'DEFERRED' ? '4 6' : '');
          const a = project(coords[0]); const b = project(coords[coords.length - 1]);
          line([coords[0][0], coords[0][1]], [coords[0][0], coords[0][1]], color, 1);
          circle(coords[0][0], coords[0][1], 5, '#FFFFFF', color, 2);
          circle(coords[coords.length - 1][0], coords[coords.length - 1][1], 5, '#FFFFFF', color, 2);
          void a; void b;
        } else {
          // Jobs are small track-attached occupancy markers, not free-floating map pins.
          polyline(coords, '#FFFFFF', 9, 0.95);
          polyline(coords, color, 4, 1, itemStatus === 'DEFERRED' ? '3 5' : '2 4');
          const mid = coords[Math.floor(coords.length / 2)];
          circle(mid[0], mid[1], 4.5, '#FFFFFF', color, 2);
          circle(mid[0], mid[1], 1.8, color);
        }
      });

      // Railway station / control points.
      PUNE_LNL_STATIONS.forEach((station) => {
        circle(station.lng, station.lat, 7, '#FFFFFF', '#173E6C', 2);
        circle(station.lng, station.lat, 3, '#173E6C');
      });

      // Chainage reference marks become useful at operational zoom levels.
      if (map.getZoom() >= 12.2) {
        const start = 191; const end = 254; const step = 2;
        for (let km = start; km <= end; km += step) {
          const p = getCoordinatesForKm(km, 'PUNE-LNL-UP');
          const q = getCoordinatesForKm(km, 'PUNE-LNL-DN');
          line(p, q, '#718294', 1, '2 3');
        }
      }
    };
    renderOverlayRef.current = renderOperationalOverlay;
    ...
    const updateActivityLabels = () => {
      ...
      activityFeaturesRef.current.forEach((feature, index) => {
        const coords = feature.geometry.coordinates; const mid = coords[Math.floor(coords.length / 2)]; const p = map.project(mid);
        const isBlock = feature.properties.entity_type === 'BLOCK';
        const el = document.createElement('button'); el.type = 'button';
        el.textContent = isBlock ? `${feature.properties.block_id} · ${feature.properties.job_count || 0}J` : feature.properties.job_id;
        const critical = !isBlock && feature.properties.criticality === 'CRITICAL';
        const offset = (index % 3 - 1) * 14;
        el.style.cssText = `position:absolute;left:0;top:0;z-index:8;pointer-events:auto;transform:translate(${p.x + offset}px,${p.y - (isBlock ? 18 : 13)}px) translate(-50%,-50%);background:rgba(255,255,255,.97);border:1px solid ${feature.properties.color};border-radius:3px;padding:2px 4px;font:700 ${isBlock ? 8 : 7}px/11px Arial,sans-serif;color:#1F2933;box-shadow:0 1px 3px rgba(0,0,0,.18);white-space:nowrap;${critical ? 'box-shadow:0 0 0 1px #C92A2A,0 1px 4px rgba(0,0,0,.22);' : ''}`;
    ...
    map.once('load', async () => {
      try {
        ...
        try {
          ...
          if (response.ok) { const railway = await response.json(); const lines = railway?.features?.filter((f) => f.geometry?.type === 'LineString') || []; if (lines.length) railwayRef.current = lines; }
        } catch (error) { console.warn('Detailed railway geometry unavailable; fixed corridor alignment retained.', error); }
        stationLabelRefs.current = PUNE_LNL_STATIONS.map((station) => {
          const el = document.createElement('button'); el.type = 'button'; el.textContent = station.code; el.title = `${station.name} • ${station.km.toFixed(2)} km`;
          el.style.cssText = 'position:absolute;left:0;top:0;z-index:6;pointer-events:auto;background:rgba(255,255,255,.96);border:1px solid #B8C6D3;border-radius:3px;padding:2px 4px;font:700 9px/12px Arial,sans-serif;color:#173E6C;box-shadow:0 1px 2px rgba(0,0,0,.14);cursor:pointer;white-space:nowrap;';
        ...
        map.on('move', refreshOverlay); map.on('resize', refreshOverlay); map.on('zoom', refreshOverlay);
        map.fitBounds(CORRIDOR_BOUNDS, { padding: { top: 80, right: 90, bottom: 80, left: 90 }, duration: 0 }); requestAnimationFrame(refreshOverlay); setLoaded(true); setMapError(null);
    ...
    return () => { stationLabelRefs.current.forEach(({ el }) => el.remove()); activityLabelRefs.current.forEach(({ el }) => el.remove()); overlayRef.current?.remove(); stationLabelRefs.current = []; activityLabelRefs.current = []; overlayRef.current = null; renderOverlayRef.current = null; window.removeEventListener('resize', resize); map.remove(); mapRef.current = null; };
  }, []);

  useEffect(() => { if (mapRef.current && overlayRef.current) requestAnimationFrame(() => renderOverlayRef.current?.()); }, [activityFeatures]);
  useEffect(() => { const map = mapRef.current; if (!map) return undefined; const refresh = () => map.resize(); requestAnimationFrame(refresh); const a = setTimeout(refresh, 150); const b = setTimeout(refresh, 500); return () => { clearTimeout(a); clearTimeout(b); }; }, [isFullScreenMode]);
  ...
  const shell = <div className={`relative overflow-hidden border border-[#C8D2DC] bg-[#DCE5E9] shadow-sm ${isFullScreenMode ? 'fixed inset-0 z-[2147483000] h-screen w-screen rounded-none' : 'h-[calc(100vh-245px)] min-h-[620px] rounded-lg'}`}>
    ...
    <div className="absolute right-4 top-[92px] z-20 rounded-md border border-[#D6DEE6] bg-white/95 px-3 py-2 text-[9px] font-semibold text-[#52606D] shadow-md"><div className="mb-1 font-bold uppercase tracking-wider text-[#8796A5]">Operational layers</div><div>● Track alignment &nbsp; ● Possession block &nbsp; ◇ Job</div><div className="mt-1">○ Station &nbsp; ┄ Chainage reference</div></div>

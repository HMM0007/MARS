...
const createDeptNavItems = (deptId) => {
  ...
  return [
    { to: basePath, icon: Home, label: 'Home', end: true },
    {
      ...
      children: [
        { to: basePath, label: 'Job Inbox' },
        { to: basePath, label: '+ Add Job Request', action: 'add-job' },
      ],
    },
    {
      key: 'schedule',
      ...
      label: 'Schedule & Map',
      children: [
        { to: `${basePath}#department-timeline-section`, label: 'Department Timeline' },
    ...
    },
    {
      key: 'coordination',
      icon: Link2,
      label: 'Coordination & Safety',
      children: [
        { to: `${basePath}?filter=scheduled`, label: 'Shared Possessions' },
        { to: '/compliance', label: 'Safety & TSR Rules' },
      ],
    },
    {
      key: 'reports',
      ...
      children: [
        { to: '/compliance', label: 'Department Impact' },
        { to: '/integration', label: 'System Status' },
      ],
    },
  ];
};
...
const deptItems = {
  engineering: createDeptNavItems('engineering'),
  snt: createDeptNavItems('snt'),
  traction: createDeptNavItems('traction'),
};
  ...
  return (
    <div className="mb-1">
      ...
      >
        ...
        {expanded ? <ChevronDown className="h-3.5 w-3.5 text-[#718294]" /> : <ChevronRight className="h-3.5 w-3.5 text-[#718294]" />}
      </button>
      {expanded && (
        <div className="ml-[21px] mt-0.5 border-l border-[#D9E1E8] pl-2">
          {item.children.map((child) => {
            if (child.action === 'add-job') {
              return (
                <button
                  key={child.label}
                  type="button"
                  onClick={() => window.dispatchEvent(new CustomEvent('mars:open-add-job'))}
                  className="flex h-[34px] w-full items-center rounded-md px-3 text-left text-[11px] font-bold text-[#1769D4] transition-colors hover:bg-[#EEF4FB]"
                >
                  {child.label}
                </button>
              );
...
export default function Sidebar({ currentRole }) {
  ...
  const navLabel = isPlanner ? 'Planner Navigation' : 'DEPARTMENT CONTROL CENTRE';
  return (
    <aside className="sticky top-[105px] flex h-[calc(100vh-105px)] w-[236px] shrink-0 flex-col border-r border-[#D8E0E8] bg-white">
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <div className="mb-3 px-2 text-[9px] font-bold uppercase tracking-[0.18em] text-[#8796A5]">
          {navLabel}
        </div>
        {items.map((item) =>
          item.children ? (

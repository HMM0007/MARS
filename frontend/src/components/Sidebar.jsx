...
import {
  LayoutDashboard, Calendar, CalendarDays, Wrench, Radio, Zap,
  BarChart3, Network, Settings, HelpCircle
} from 'lucide-react';
...
const Sidebar = () => {
    ...
    return (
      <li key={item.to}>
        <NavLink
          to={item.to}
          className={({ isActive }) => `
            flex items-center space-x-3 ${compact ? 'px-4 py-2' : 'px-4 py-3'} rounded-md text-sm
            transition-colors
            ${isActive
              ? 'bg-[#1E3A5F] text-white'
              : 'text-[#1F2933] hover:bg-white hover:shadow-sm'}
          `}
        >
          {({ isActive }) => (
            <>
  ...
  return (
    <aside className="w-64 bg-[#F4F6F8] border-r border-[#D6DEE6] min-h-screen relative">

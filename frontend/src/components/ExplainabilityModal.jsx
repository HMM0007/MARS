      ...
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-md border border-[#D6DEE6] bg-white shadow-2xl">
        ...
        <div className="overflow-y-auto p-5 text-xs">
          <section className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
            ...
            <div className="rounded border border-[#D6DEE6] bg-[#F8FAFC] p-3"><span className="text-[9px] font-bold uppercase text-[#718294]">Departments</span><p className="mt-1 font-bold text-[#1F2933]">{departmentSummary}</p></div>
          ...
          <section className="mt-4 rounded border border-[#2F6F7E]/25 bg-[#2F6F7E]/5 p-3.5">
            ...
            <p className="mt-1.5 leading-relaxed text-[#1F2933]">{block.explanation || `MARS scheduled this ${isConsolidated ? 'shared ' : ''}possession on ${block.track_id} while protecting train movements and mandatory safety constraints.`}</p>
          ...
          <section className="mt-5">
            ...
            {jobs.length ? <div className="overflow-hidden rounded border border-[#D6DEE6]"><table className="w-full text-left"><thead className="bg-[#F4F6F8] text-[9px] font-bold uppercase text-[#52606D]"><tr><th className="px-3 py-2">Job</th><th className="px-3 py-2">Department</th><th className="px-3 py-2">Work / Defect</th><th className="px-3 py-2">Criticality</th><th className="px-3 py-2">AI Priority</th></tr></thead><tbody className="divide-y divide-[#E5EAF0]">{jobs.map((job) => <tr key={job.job_id} className="hover:bg-[#F8FAFC]"><td className="px-3 py-2 font-mono font-bold text-[#1E3A5F]">{job.job_id}</td><td className="px-3 py-2 font-semibold">{job.department || '—'}</td><td className="px-3 py-2 text-[#52606D]">{String(job.defect_type || job.work_type || 'Maintenance').replace(/_/g, ' ')}</td><td className="px-3 py-2"><span className="rounded bg-[#F4F6F8] px-1.5 py-0.5 text-[9px] font-bold">{job.criticality_level || '—'}</span></td><td className="px-3 py-2 font-mono font-bold">{job.ai_priority_score ?? '—'}</td></tr>)}</tbody></table></div> : <div className="rounded border border-[#C92A2A]/25 bg-[#C92A2A]/5 p-3 text-[10px] font-semibold text-[#C92A2A]">No associated job records were supplied by the weekly-plan API for this possession.</div>}
          </section>

          <section className="mt-5"><div className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-[#1F2933]"><ShieldCheck className="h-4 w-4 text-[#2F9E44]" /> Safety verification</div><div className="divide-y divide-[#E5EAF0] rounded border border-[#D6DEE6]">{hardConstraints.map(([name, detail]) => <div key={name} className="flex items-center justify-between gap-3 px-3 py-2.5"><div><p className="font-bold">{name}</p><p className="mt-0.5 text-[10px] text-[#718294]">{detail}</p></div><span className="rounded border border-[#2F9E44]/25 bg-[#2F9E44]/10 px-2 py-0.5 text-[9px] font-bold text-[#2F9E44]">VERIFIED</span></div>)}</div></section>

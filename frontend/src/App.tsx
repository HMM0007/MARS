...
const navItems: { label: string; icon: IconName; section?: string }[] = [
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <div className="footer-version">MARS · Prototype</div>
        </div>
      </aside>
        ...
        <header className="topbar">
          <div className="topbar-title">
            ...
            <div>
              ...
              <p>Maintenance planning and operational status</p>
            </div>
          </div>
          <div className="topbar-actions">
            <div className="plan-state"><span /> Current Active Plan: <strong>ACTIVE</strong></div>
            <button className="icon-button" aria-label="Refresh">↻</button>
            <button className="icon-button notification" aria-label="Notifications">♢<b>3</b></button>
            <div className="user-chip"><div className="avatar">PA</div><div><strong>Planner Admin</strong><small>Divisional Planner</small></div><span>⌄</span></div>
          </div>
        </header>

        <div className="content">
          <section className="page-heading">
            <div><span className="eyebrow">MARS / OPERATIONS</span><h2>Current Maintenance Plan</h2></div>
            <div className="date-control">30 Aug 2026 <span>⌄</span></div>
          </section>

          <section className="metric-strip" aria-label="Operational summary">
            <Metric label="Total Jobs" value="40" detail="Maintenance requirements" />
            <Metric label="Scheduled Jobs" value="11" detail="27.5% of total" />
            <Metric label="Unscheduled Jobs" value="29" detail="Require planning" />
            <Metric label="Critical Jobs" value="6" detail="Require attention" tone="critical" />
            <Metric label="Available Blocks" value="20" detail="Maintenance windows" />
            <Metric label="Active Sections" value="10" detail="Network sections" />
          </section>

          <section className="workspace-grid">
            <Panel title="Railway Operations" action="Open network">
              <div className="railway-view">
                <div className="railway-meta"><span>SCHEMATIC NETWORK</span><span>10 sections · 20 blocks</span></div>
                <div className="railway-line">
                  <Node code="S01" name="Section 01" status="active" />
                  <Segment />
                  <Node code="S02" name="Section 02" status="clear" />
                  <Segment />
                  <Node code="S03" name="Section 03" status="active" />
                  <Segment warning />
                  <Node code="S05" name="Section 05" status="work" />
                  <Segment />
                  <Node code="S07" name="Section 07" status="clear" />
                </div>
                <div className="railway-legend"><span><i className="legend-active" /> Active</span><span><i className="legend-work" /> Maintenance</span><span><i className="legend-warning" /> Attention</span></div>
              </div>
            </Panel>

            <Panel title="Operational Status" action="View details">
              <div className="status-list">
                <StatusRow label="Scheduled maintenance" value="11 jobs" tone="ok" />
                <StatusRow label="Pending planning" value="29 jobs" tone="warn" />
                <StatusRow label="Active conflicts" value="0" tone="ok" />
                <StatusRow label="Critical work" value="6 jobs" tone="critical" />
                <StatusRow label="Current plan" value="Active" tone="ok" />
              </div>
            </Panel>
          </section>

          <section className="section-block">
            <SectionHeader title="Maintenance Planning" subtitle="Current Active Plan · Scheduled work" action="View complete plan" />
            <div className="toolbar">
              <button className="filter">Status: All <span>⌄</span></button>
              ...
              <div className="search">⌕ <span>Search jobs</span></div>
            </div>
            <div className="table-wrap">
              <table>
                <thead><tr><th>JOB ID</th><th>SECTION</th><th>BLOCK</th><th>PLANNED TIME</th><th>PRIORITY</th><th>STATUS</th><th /></tr></thead>
                <tbody>{jobs.map((job) => <tr key={job[0]}><td className="job-id">{job[0]}</td><td>{job[1]}</td><td>{job[2]}</td><td>{job[3]}</td><td><Priority value={job[4]} /></td><td><span className="status-badge scheduled">{job[5]}</span></td><td className="row-arrow">›</td></tr>)}</tbody>
              </table>
            </div>
          </section>

          <section className="lower-grid">
            <Panel title="Maintenance Timeline" action="Open Gantt">
              <div className="mini-gantt">
                <div className="gantt-head"><span>SECTION / BLOCK</span><div>06:00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 08:00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 10:00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 12:00</div></div>
                <GanttRow label="S01 / B001" bar="j1" start="8%" width="24%" text="J001" />
                <GanttRow label="S03 / B004" bar="j2" start="28%" width="22%" text="J002" />
                <GanttRow label="S05 / B020" bar="critical" start="8%" width="48%" text="J004 · J036" />
                <GanttRow label="S07 / B019" bar="j4" start="50%" width="30%" text="J029" />
              </div>
            </Panel>
            <Panel title="Re-planning" action="Open workspace">
              <div className="replan-box">
                <div className="replan-title"><span className="alert-mark">!</span><div><strong>Operational change required?</strong><p>Run impact analysis and generate a revised plan.</p></div></div>
                <div className="replan-steps"><span>1&nbsp; Event</span><i>→</i><span>2&nbsp; Impact</span><i>→</i><span>3&nbsp; Re-plan</span><i>→</i><span>4&nbsp; Review</span></div>
                <button className="primary-button">Open Re-planning</button>
              </div>
            </Panel>
          </section>
        </div>
...
function Panel({ title, action, children }: { title: string; action?: string; children: React.ReactNode }) {
  return <div className="panel"><div className="panel-header"><h3>{title}</h3>{action && <button>{action} <span>›</span></button>}</div>{children}</div>
}
function SectionHeader({ title, subtitle, action }: { title: string; subtitle: string; action: string }) {
  return <div className="section-header"><div><h3>{title}</h3><p>{subtitle}</p></div><button>{action} <span>›</span></button></div>
}
